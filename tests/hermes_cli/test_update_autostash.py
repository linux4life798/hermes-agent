...
# ---------------------------------------------------------------------------
# rebase update behavior for diverged history
# ---------------------------------------------------------------------------
...
def _make_update_side_effect(
    ...
    def side_effect(cmd, **kwargs):
        ...
        if "rebase" in joined and "--abort" in joined:
            if abort_fails:
                return SimpleNamespace(stdout="", stderr="fatal: No rebase in progress?\n", returncode=1)
            return SimpleNamespace(stdout="", stderr="", returncode=0)
        return SimpleNamespace(returncode=0, stdout="", stderr="")
...
def test_cmd_update_aborts_rebase_without_reset_when_pull_rebase_fails(monkeypatch, tmp_path, capsys):
    """When pull --rebase fails, update aborts the rebase and preserves local commits."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
    ...
    assert ["git", "pull", "--rebase", "origin", "main"] in recorded
    assert ["git", "rebase", "--abort"] in recorded
    assert [c for c in recorded if "reset" in c and "--hard" in c and "origin/main" in c] == []
    assert [c for c in recorded if "pip" in c and "install" in c] == []

    out = capsys.readouterr().out
    assert "Rebase failed" in out
    assert "local commits were preserved" in out


def test_cmd_update_uses_rebase_and_no_reset_when_rebase_succeeds(monkeypatch, tmp_path):
    """Successful updates pull with rebase and do not reset to origin/main."""
    _setup_update_mocks(monkeypatch, tmp_path)
    monkeypatch.setattr("shutil.which", lambda name: "/usr/bin/uv" if name == "uv" else None)
    ...
    assert ["git", "pull", "--rebase", "origin", "main"] in recorded
    reset_calls = [c for c in recorded if "reset" in c and "--hard" in c and "origin/main" in c]
...
# ---------------------------------------------------------------------------
# rebase abort failure — don't attempt stash restore
# ---------------------------------------------------------------------------

def test_cmd_update_skips_stash_restore_when_rebase_abort_fails(monkeypatch, tmp_path, capsys):
    """When rebase abort fails, stash restore is skipped with manual recovery guidance."""
    _setup_update_mocks(monkeypatch, tmp_path)
    ...
    side_effect, recorded = _make_update_side_effect(rebase_fails=True, abort_fails=True)
    monkeypatch.setattr(hermes_main.subprocess, "run", side_effect)

    with pytest.raises(SystemExit, match="1"):
        hermes_main.cmd_update(SimpleNamespace())

    # Stash restore should NOT have been called while rebase state is uncertain.
    assert len(restore_calls) == 0
    assert ["git", "rebase", "--abort"] in recorded

    out = capsys.readouterr().out
    assert "Failed to abort the rebase" in out
